...
complete -c tgrep -l colors -d 'set whether to color output' -r -f -a "{always	'',never	''}"
complete -c tgrep -s s -l searcher -d 'executable to do the searching, currently supports rg, grep and tgrep' -r
...
complete -c tgrep -s f -l files -d 'show paths that have matches'
complete -c tgrep -l links -d 'show linked paths for symbolic links'
complete -c tgrep -l trim-left -d 'trim whitespace at beginning of lines'
