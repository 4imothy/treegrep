...
function __fish_tgrep_global_optspecs
	string join \n e/regexp= p/path= glob= s/searcher= threads= ./hidden n/line-number f/files links trim pcre2 no-ignore c/count no-color no-bold overview max-depth= prefix-len= max-length= long-branch-each= char-style= long-branch m/menu h/help V/version
end
